...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!who) throw m.reply('Tag salah satu lah, yang kamu ingin berdagang bareng')
    let ctimer = (new Date - user.lastdagang)
    let _ctimer = (cooldown - ctimer)
    let timers = clockString(_ctimer)
    if (user.lastdagang < cooldown) {
        if (5000 > user.money) throw m.reply('Target tidak memiliki modal harap masukkan modal 5000')
        if (5000 > user.money) throw m.reply('kamu tidak memiliki modal harap masukkan modal 5000')
