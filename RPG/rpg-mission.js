...
let misi = JSON.parse(readFileSync('./src/misi.json'))
...
- Cancel
`
      return conn.reply(m.chat, caption, `ketik .accept untuk terima\n\nketik cancel jiga ingin di canel`, m) //SendMessage
    }
  } catch (e) {
    console.error(e)
    if(m.sender in conn.mission) {
